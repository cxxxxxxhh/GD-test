...
class Game(object):
    ...
    def drawScoreboard(self, screen, game):
        ...
        nowlevel = font.render(f"Now Level: {self.wildRank}",True,(255,130,0))
        screen.blit(teamAText, (30, 20))
        screen.blit(teamBText, (30, 50))
        screen.blit(nowlevel, (30,80))
    ...
    def drawclock(self,screen,mousePos,game):
        font = pygame.font.Font(None, 24)
        self.clock=pygame.draw.circle(center=(WIDTH/2-230,500),radius=10)
...
class Player(object):
    ...
    def select(self, game, event):
        if game.players.index(self) == 0:#########################################添加计时器
            # start_time = time.time()
